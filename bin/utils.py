...
def zip_files(zip_file_name, saved_dir, files_to_zip):
    with zipfile.ZipFile(os.path.join(saved_dir, zip_file_name), 'x') as zipMe:
        for file in files_to_zip:
            zipMe.write(file, path.basename(file), compress_type=zipfile.ZIP_DEFLATED)
...
def download_file(dir_to_down, settings, change):
    ...
    print(dir_to_down)
    shutil.copy(path.join(sanitize_path(settings['zip_dir']),change['package_name']), dir_to_down)
